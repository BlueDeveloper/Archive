...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await db().insert(workHours).values(body as never).returning();
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const { id, ...data } = body;
    const result = await db().update(workHours).set(data as never).where(eq(workHours.id, id as number)).returning();
