...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await db().insert(projects).values(body as never).returning();
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const result = await db().update(projects).set(data as never).where(eq(projects.id, id as number)).returning();
